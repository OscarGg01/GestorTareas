...
from src.logica.gestion import GestionTareas
...
class GestionApp:
    ...
    def agregar_tarea(self):
        ...
        try:
            self.gestion.agregar_tarea(titulo, descripcion)
            self.actualizar_lista()
        except ValueError as e:
            messagebox.showerror("Error", str(e))
    def actualizar_lista(self):
        self.tareas_listbox.delete(0, tk.END)
        for indice, tarea in enumerate(self.gestion.obtener_tareas()):
    ...
    def marcar_completada(self):
        ...
        if seleccion:
            indice = seleccion[0]
            self.gestion.marcar_completada(indice)
            self.actualizar_lista()
        else:
